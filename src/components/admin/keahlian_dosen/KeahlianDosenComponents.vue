<template>
  <div class="relative w-full">
    ...
    <div class="flex flex-col gap-5 mb-6 p-5 rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] shadow-sm relative z-20">
      ...
      <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3 w-full sm:w-auto">
          <button v-if="isAdmin" @click="syncAllScholar" :disabled="isSyncingAll"
            class="flex-1 sm:flex-none flex items-center justify-center px-4 h-10 text-sm font-medium text-brand-600 transition rounded-lg bg-brand-50 border border-brand-200 hover:bg-brand-100 shadow-theme-xs disabled:opacity-50">
      ...
      <div class="flex flex-col lg:flex-row items-center gap-4">
          ...
          <transition enter-active-class="transition duration-100 ease-out" enter-from-class="transform scale-95 opacity-0" enter-to-class="transform scale-100 opacity-100" leave-active-class="transition duration-75 ease-in" leave-from-class="transform scale-100 opacity-100" leave-to-class="transform scale-95 opacity-0">
            <div v-if="isGrupRisetDropdownOpen" class="absolute z-[100] w-full mt-1.5 top-full bg-white border border-gray-200 rounded-lg shadow-xl dark:bg-gray-800 dark:border-gray-700 overflow-hidden">
              <ul class="max-h-60 overflow-y-auto custom-scrollbar py-1">
                <li @click="selectFilterGrupRiset('')" class="px-4 py-2.5 text-sm cursor-pointer hover:bg-brand-50 dark:hover:bg-gray-700 text-gray-800 dark:text-gray-200 transition-colors border-b border-gray-100 dark:border-gray-700 font-medium">
                  Semua Grup Riset
                </li>
                <li v-for="grup in grupRisetList" :key="grup" @click="selectFilterGrupRiset(grup)" class="px-4 py-2.5 text-sm cursor-pointer hover:bg-brand-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 transition-colors border-b border-gray-50 last:border-0 dark:border-gray-700/50">
                  {{ grup }}
                </li>
              </ul>
            </div>
        ...
        <div class="relative w-full lg:w-1/3" ref="topikFilterDropdownRef">
          ...
          <transition enter-active-class="transition duration-100 ease-out" enter-from-class="transform scale-95 opacity-0" enter-to-class="transform scale-100 opacity-100" leave-active-class="transition duration-75 ease-in" leave-from-class="transform scale-100 opacity-100" leave-to-class="transform scale-95 opacity-0">
            <div v-if="isTopikFilterDropdownOpen" class="absolute z-[100] w-full mt-1.5 top-full bg-white border border-gray-200 rounded-lg shadow-xl dark:bg-gray-800 dark:border-gray-700 overflow-hidden">
              <ul class="max-h-60 overflow-y-auto custom-scrollbar py-1">
                <li @click="selectFilterTopik('')" class="px-4 py-2.5 text-sm cursor-pointer hover:bg-brand-50 dark:hover:bg-gray-700 text-gray-800 dark:text-gray-200 transition-colors border-b border-gray-100 dark:border-gray-700 font-medium">
                  Semua Topik Keahlian
                </li>
                <li v-for="topik in topikList" :key="topik.id_topik" @click="selectFilterTopik(topik.id_topik)" class="px-4 py-2.5 text-sm cursor-pointer hover:bg-brand-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 transition-colors border-b border-gray-50 last:border-0 dark:border-gray-700/50">
                  {{ topik.nama_topik }}
                </li>
              </ul>
            </div>
        ...
        <div class="relative w-full lg:w-1/3">
          <input v-model="searchKeahlianQuery" type="text" placeholder="Cari nama dosen..." class="h-10 w-full rounded-lg border border-gray-300 bg-white pl-10 pr-4 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:outline-none focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" />
    ...
    <!-- Tabel Data -->
    <div class="rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] shadow-sm relative z-10">
        ...
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tr v-else v-for="(group, index) in paginatedGroups" :key="group.dosen_id" class="border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-white/[0.02]">
              ...
              <td class="px-5 py-4 sm:px-6 align-middle">
                <span class="block font-semibold text-gray-800 text-theme-sm dark:text-white/90">{{ group.nama_dosen }}</span>
                <span class="block text-[11px] text-gray-500 mt-1">{{ group.grup_riset || 'Tidak Ada Grup' }}</span>
              </td>

              <!-- TOPIK KEAHLIAN & TOOLTIP DI TABEL -->
              <td class="px-5 py-4 sm:px-6 align-middle">
                <div class="flex flex-wrap gap-2">
                  <div v-for="keahlian in group.keahlian" :key="keahlian.id_keahlian" class="group relative inline-flex justify-center">

                    <span class="inline-flex items-center gap-1.5 justify-center rounded-md bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-700 border border-blue-200 cursor-help transition-all hover:bg-blue-100 hover:border-blue-300 shadow-sm relative z-10">
                        ...
                        <div class="relative z-10 p-3 bg-white dark:bg-gray-800 rounded-b-xl">
                          <ul v-if="!keahlian.keyword_jurnal.includes('Manual')" class="flex flex-col gap-2.5">
                            <li v-for="(jurnal, i) in parseJurnal(keahlian.keyword_jurnal)" :key="i" class="text-[11px] text-gray-600 dark:text-gray-400 leading-snug flex items-start gap-2">
                              <span v-if="!jurnal.startsWith('...')" class="text-blue-500 mt-[3px] text-[8px]">●</span>
                              <span :class="['text-left font-medium', { 'italic text-blue-500 font-semibold text-center w-full mt-1': jurnal.startsWith('...') }]">{{ jurnal }}</span>
                            </li>
                          </ul>
              ...
              <td v-if="isAdmin" class="px-5 py-4 text-center sm:px-6 align-middle">
                <div class="flex justify-center gap-2">
                  <button @click="syncOneScholar(group.dosen_id)" :disabled="isSyncingId === group.dosen_id" class="flex items-center justify-center w-8 h-8 text-brand-600 bg-brand-50 border border-brand-200 hover:bg-brand-100 rounded-full transition-colors disabled:opacity-50" title="Sync Scholar">
    ...
    <Pagination v-if="!isLoading && totalItems > 0" :current-page="currentPage" :total-items="totalItems" :items-per-page="itemsPerPage" @update:currentPage="currentPage = $event" class="mt-4" />

    <!-- MODAL TAMBAH/EDIT KEAHLIAN DOSEN -->
        ...
        <form @submit.prevent="submitForm" class="flex flex-col px-2 pb-10">
          <div class="mb-5 relative z-[60]" ref="dosenDropdownContainer">
            ...
            <div v-if="!isEditing" class="relative">
              <input type="text" v-model="dosenSearchTerm" @focus="showDosenDropdown = true" placeholder="Ketik nama dosen..." class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:outline-none focus:ring-3 focus:ring-brand-500/10" required />
              <ul v-if="showDosenDropdown && filteredDosenOptions.length > 0" class="absolute w-full mt-1 max-h-60 overflow-y-auto bg-white dark:bg-gray-800 border border-gray-200 rounded-lg shadow-xl">
                <li v-for="dosen in filteredDosenOptions" :key="dosen.id_dosen" @click="selectDosen(dosen)" class="px-4 py-2 cursor-pointer hover:bg-brand-50 text-sm text-gray-800 border-b border-gray-100 last:border-0">{{ dosen.nama_dosen }}</li>
              </ul>
            </div>
          ...
          <div class="mb-6 relative z-[50]" ref="topikDropdownContainer">
            ...
            <div @click="showTopikDropdown = !showTopikDropdown"
                 class="min-h-[44px] w-full rounded-lg border border-gray-300 bg-white px-2 pr-10 py-1.5 text-sm text-gray-800 shadow-theme-xs cursor-pointer flex flex-wrap gap-2 items-center relative transition-colors focus-within:border-brand-300 dark:bg-gray-900 dark:border-gray-700">
          ...
          <div class="flex items-center gap-3 lg:justify-end mt-4">
            <button @click="closeModal" type="button" class="flex w-full justify-center rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 sm:w-auto">Batal</button>
            <button type="submit" :disabled="isSaving || !formData.dosen_id || formData.topik_ids.length === 0" class="flex w-full justify-center rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-brand-600 disabled:opacity-50 sm:w-auto">{{ isSaving ? 'Menyimpan...' : 'Simpan' }}</button>
          </div>
        </form>
      </div>
    </Modal>
