<template>
  <div class="relative">
      ...
      <div class="max-w-full overflow-x-auto custom-scrollbar">
        <table class="min-w-full">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tr v-else-if="keahlianList.length === 0">
              <td colspan="4" class="px-5 py-8 text-center text-gray-500 text-theme-sm">
                Belum ada data keahlian dosen.
              </td>
            </tr>
            <tr v-else v-for="(item, index) in keahlianList" :key="item.id"
              ...
              <td class="px-5 py-4 sm:px-6">
                <span class="inline-flex items-center justify-center rounded-full bg-blue-100 px-3 py-1 text-xs font-medium text-blue-800">
              ...
              <td class="px-5 py-4 text-center sm:px-6">
                <div class="flex items-center justify-center gap-2">
                  ...
                  <button @click="openDeleteModal(item)"
                    class="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-gray-700 transition-colors bg-white border border-gray-200 rounded-full hover:bg-gray-50 hover:text-red-600 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-red-400 shadow-theme-xs" title="Hapus">
    ...
    <Modal v-if="isModalOpen" @close="closeModal">
        <div class="no-scrollbar relative w-full max-w-[500px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11 mx-auto mt-10">
          <button @click="closeModal" class="transition-color absolute right-5 top-5 z-999 flex h-11 w-11 items-center justify-center rounded-full bg-gray-100 text-gray-400 hover:bg-gray-200 hover:text-gray-600 dark:bg-white/[0.05] dark:text-gray-400 dark:hover:bg-white/[0.07] dark:hover:text-gray-300">
          ...
          <form @submit.prevent="submitForm" class="flex flex-col px-2">
            ...
            <div class="mb-5">
              ...
              <select v-model="formData.dosen_id" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800" required :disabled="isEditing">
                <option value="" disabled>Pilih Dosen...</option>
                <option v-for="dosen in dosenList" :key="dosen.id" :value="dosen.id">
                  {{ dosen.nama_dosen }}
                </option>
              </select>
              <p v-if="isEditing" class="mt-1 text-xs text-gray-500">Nama dosen tidak dapat diubah saat mode edit.</p>
            </div>

            <div class="mb-6">
              <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                Pilih Topik Keahlian <span class="text-red-500">*</span>
              </label>
              <select v-model="formData.topik_id" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800" required>
                <option value="" disabled>Pilih Topik TA...</option>
    ...
    <Modal v-if="isDeleteModalOpen" @close="closeDeleteModal">
        <div class="w-full max-w-[400px] rounded-3xl bg-white p-6 dark:bg-gray-900 text-center mx-auto mt-20 z-50">
          ...
          <p class="mb-8 text-sm text-gray-500 dark:text-gray-400">
            Apakah Anda yakin ingin menghapus keahlian <br/>
            <strong class="text-gray-800 dark:text-white text-lg">"{{ itemToDelete?.topik_ta?.nama_topik }}"</strong> <br/>
            dari dosen <strong class="text-gray-800 dark:text-white">{{ itemToDelete?.dosen?.nama_dosen }}</strong>?
          </p>
...
const itemToDelete = ref<Keahlian | null>(null)
...
const fetchKeahlian = async () => {
  ...
  try {
    const response = await fetch('http://localhost:3000/api/keahlian-dosen', {
      method: 'GET',
...
const openAddModal = () => {
  ...
  formData.value = { id: null, dosen_id: '', topik_id: '' }
  isModalOpen.value = true
}

const openEditModal = (item: Keahlian) => {
...
const submitForm = async () => {
  ...
  try {
    const url = isEditing.value
      ? `http://localhost:3000/api/keahlian-dosen/${formData.value.id}`
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        topik_id: Number(formData.value.topik_id)
      })
    })
    ...
    if (response.ok) {
      ...
      // Menampilkan pesan error dari backend (seperti Dosen sudah mendaftarkan topik tersebut)
...
// --- FUNGSI DELETE MODAL ---
const openDeleteModal = (item: Keahlian) => {
...
const confirmDelete = async () => {
  ...
  try {
    const response = await fetch(`http://localhost:3000/api/keahlian-dosen/${itemToDelete.value.id}`, {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include'
    })
    ...
    if (response.ok) {
      closeDeleteModal()
      fetchKeahlian()
      showAlert('success', 'Dihapus!', 'Data Keahlian Dosen berhasil dihapus.')
    } else {
