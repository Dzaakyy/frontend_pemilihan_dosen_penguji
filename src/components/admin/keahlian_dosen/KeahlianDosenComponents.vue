<template>
  <div class="relative">
    ...
    <div
      ...
      <div class="flex flex-col lg:flex-row items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3 w-full lg:w-auto sm:justify-end">
          ...
          <!-- TOMBOL BARU: Sync 1 Dosen Manual Bebas -->
          <button v-if="isAdmin" @click="openSyncModal" :disabled="isSyncingAll"
            class="flex-1 sm:flex-none flex items-center justify-center px-4 h-10 text-sm font-medium text-blue-600 transition rounded-lg bg-blue-50 border border-blue-200 hover:bg-blue-100 shadow-theme-xs disabled:opacity-50 dark:bg-blue-900/20 dark:border-blue-800/50 dark:text-blue-400 dark:hover:bg-blue-900/40">
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-white/[0.02]">
              <th class="py-3 px-3 w-[5%] sm:w-[5%] text-center rounded-tl-xl">
                <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">No</p>
              </th>
              <th class="px-5 py-3 w-[30%] sm:w-[25%] sm:px-6">
                <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Nama Dosen & Grup</p>
              </th>
              <th class="px-5 py-3 w-[45%] sm:w-[55%] sm:px-6">
                <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Sub-Bidang Keahlian & Bukti
                  Scholar</p>
              </th>
              <th v-if="isAdmin" class="px-5 py-3 text-center w-[20%] sm:w-[15%] sm:px-6 rounded-tr-xl">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tr v-else-if="paginatedGroups.length === 0">
              <td :colspan="isAdmin ? 4 : 3"
                class="px-5 py-12 text-center text-gray-500 text-theme-sm dark:text-gray-400">
            ...
            <tr v-else v-for="(group, index) in paginatedGroups" :key="group.dosen_id"
              ...
              <td class="px-5 py-4 sm:px-6 align-middle">
                ...
                <span class="block text-[11px] text-gray-500 dark:text-gray-400 mt-1">{{ group.grup_riset || 'Tidak AdaGrup' }}</span>
              </td>

              <td class="px-5 py-4 sm:px-6 align-middle">
                <div class="flex flex-wrap gap-2 w-full max-w-full">
                  <div v-for="(keahlian, kIndex) in group.keahlian" :key="keahlian.id_keahlian"
                    ...
                    <div v-if="keahlian.keyword_jurnal"
                      class="absolute hidden group-hover:block z-[9999] w-[340px] pointer-events-none" :class="[
                        ...
                        <div class="relative z-10 p-3 bg-white dark:bg-gray-800 rounded-b-xl whitespace-normal break-words">
                          <ul v-if="!keahlian.keyword_jurnal.includes('Manual')" class="flex flex-col gap-2.5">
                            <li v-for="(jurnal, i) in parseJurnal(keahlian.keyword_jurnal)" :key="i"
                              ...
                              <span v-if="!jurnal.startsWith('...')"
                                class="text-blue-500 dark:text-blue-400 mt-[3px] text-[8px]">●</span>
                              <span
                                :class="['text-left font-medium w-full', { 'italic text-blue-500 dark:text-blue-400 font-semibold text-center mt-1': jurnal.startsWith('...') }]">{{
                                jurnal }}</span>
                            </li>
                          </ul>
    ...
    <!-- MODAL BARU: Sync 1 Dosen dari Luar Table -->
    <Modal v-if="isSyncModalOpen" @close="closeSyncModal">
      <div
        class="relative w-full max-w-[500px] flex flex-col rounded-3xl bg-white dark:bg-gray-900 border dark:border-gray-700 mx-auto mt-10 shadow-2xl overflow-visible">

        <div
          class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 z-10 shrink-0 rounded-t-3xl">
          <h4 class="text-xl font-bold text-gray-800 dark:text-white/90">Sync Scholar 1 Dosen</h4>
        ...
        <div class="px-6 py-8 overflow-visible flex-1">
          <div class="relative z-[70]" ref="dosenSyncDropdownContainer">
            <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Pilih Dosen (Ketik untuk mencari) <span class="text-red-500">*</span></label>
            <div class="relative">
              <input type="text" v-model="dosenSyncSearchTerm" @focus="showDosenSyncDropdown = true"
                placeholder="Ketik nama dosen..."
                class="dark:bg-gray-800 h-12 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:outline-none focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:text-gray-200" />
              <ul v-if="showDosenSyncDropdown && filteredSyncDosenOptions.length > 0"
                class="absolute z-[100] w-full mt-1 max-h-60 overflow-y-auto bg-white dark:bg-gray-800 border border-gray-200 rounded-lg shadow-xl dark:border-gray-700 custom-scrollbar">
                <li v-for="dosen in filteredSyncDosenOptions" :key="dosen.id_dosen" @click="selectSyncDosen(dosen)"
                  class="px-4 py-3 cursor-pointer hover:bg-brand-50 text-sm text-gray-800 border-b border-gray-100 last:border-0 dark:text-gray-200 dark:hover:bg-gray-700 dark:border-gray-700">
                  {{ dosen.nama_dosen }}
                </li>
              </ul>
              <div v-if="showDosenSyncDropdown && filteredSyncDosenOptions.length === 0"
                class="absolute z-[100] w-full mt-1 px-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 rounded-lg shadow-xl dark:border-gray-700 text-sm text-gray-500 italic text-center">
                Dosen tidak ditemukan atau sudah tersinkron.
              </div>
            </div>
            <p class="mt-2 text-xs text-gray-500">Menampilkan dosen yang <strong>belum memiliki keahlian</strong> dan <strong>memiliki link Scholar</strong>.</p>
          </div>

          <div class="flex items-center gap-3 lg:justify-end mt-10 pt-4 border-t border-gray-100 dark:border-gray-800 shrink-0">
            <button @click="closeSyncModal" type="button"
              class="flex w-full justify-center rounded-lg border border-gray-300 bg-white px-5 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 sm:w-auto dark:bg-gray-800 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-700">Batal</button>
            <button @click="executeSyncSingle" :disabled="!selectedDosenToSync || isSyncingId === selectedDosenToSync"
              class="flex w-full justify-center rounded-lg bg-brand-500 px-5 py-3 text-sm font-medium text-white hover:bg-brand-600 disabled:opacity-50 sm:w-auto dark:bg-brand-600 dark:hover:bg-brand-500">
              {{ isSyncingId === selectedDosenToSync ? 'Menyinkronkan...' : 'Sync Sekarang' }}
            </button>
          </div>
        </div>
      </div>
    ...
    <Modal v-if="isModalOpen" @close="closeModal">
      <!-- max-w diubah ke 800px agar lebih lega, overflow-visible agar dropdown bisa keluar dari batas -->
      <div
        class="relative w-full max-w-[800px] flex flex-col rounded-3xl bg-white dark:bg-gray-900 border dark:border-gray-700 mx-auto mt-10 shadow-2xl overflow-visible">
        ...
        <div class="px-6 py-10 overflow-visible flex-1">
          <form @submit.prevent="submitForm" class="flex flex-col gap-8">

            <div class="relative z-[70]" ref="dosenDropdownContainer">
              <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Pilih Dosen (Ketik untuk mencari) <span class="text-red-500">*</span></label>
              ...
              <input v-else type="text" :value="dosenSearchTerm" disabled
                class="dark:bg-gray-800 h-12 w-full rounded-lg border border-gray-300 bg-gray-100 px-4 py-2.5 text-sm text-gray-500 shadow-theme-xs cursor-not-allowed dark:border-gray-700 dark:text-gray-400" />
            </div>

            <div class="relative z-[60]" ref="topikDropdownContainer">
              <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Pilih Sub-Bidang Keahlian <span class="text-red-500">*</span></label>
              <div @click="showTopikDropdown = !showTopikDropdown"
                class="min-h-[50px] w-full rounded-lg border border-gray-300 bg-white px-2 pr-10 py-1.5 text-sm text-gray-800 shadow-theme-xs cursor-pointer flex flex-wrap gap-2 items-center relative transition-colors focus-within:border-brand-300 dark:bg-gray-800 dark:border-gray-700">
                ...
                <span v-for="kbk in formData.bidang_keahlian_list" :key="kbk"
                  class="inline-flex items-center gap-1.5 rounded-md bg-brand-50 pl-2.5 py-1 text-xs font-medium text-brand-700 border border-brand-200 dark:bg-brand-900/30 dark:text-brand-300 dark:border-brand-800">
                  {{ kbk }}
                  <button @click.stop="toggleKbk(kbk)" type="button"
                ...
                <div class="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-gray-400">
                  <svg class="w-5 h-5 transition-transform duration-200" :class="{ 'rotate-180': showTopikDropdown }"
                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                  </svg>
                </div>
              ...
              <transition enter-active-class="transition duration-100 ease-out"
                ...
                leave-active-class="transition duration-75 ease-in" leave-from-class="transform scale-100 opacity-100"
                leave-to-class="transform scale-95 opacity-0">
                <div v-if="showTopikDropdown"
                  class="absolute z-[100] w-full mt-1 max-h-60 overflow-y-auto bg-white border border-gray-200 rounded-lg shadow-xl dark:bg-gray-800 dark:border-gray-700 custom-scrollbar">
                  <div v-if="availableKbkList.length === 0" class="px-4 py-4 text-sm text-gray-500 italic text-center">
                  ...
                  <div v-else v-for="kbk in availableKbkList" :key="kbk" @click="toggleKbk(kbk)"
                    class="px-4 py-3.5 cursor-pointer hover:bg-brand-50 text-sm text-gray-800 border-b border-gray-100 last:border-0 flex items-center justify-between dark:hover:bg-gray-700 dark:text-gray-200 dark:border-gray-700">
                    <span>{{ kbk }}</span>
                    <svg v-if="formData.bidang_keahlian_list.includes(kbk)" class="w-5 h-5 text-brand-500" fill="none"
            ...
            <div class="flex items-center gap-3 lg:justify-end mt-10 pt-4 border-t border-gray-100 dark:border-gray-800 shrink-0">
              <button @click="closeModal" type="button"
                class="flex w-full justify-center rounded-lg border border-gray-300 bg-white px-5 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 sm:w-auto dark:bg-gray-800 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-700">Batal</button>
              <button type="submit"
                :disabled="isSaving || !formData.dosen_id || formData.bidang_keahlian_list.length === 0"
                class="flex w-full justify-center rounded-lg bg-brand-500 px-5 py-3 text-sm font-medium text-white hover:bg-brand-600 disabled:opacity-50 sm:w-auto dark:bg-brand-600 dark:hover:bg-brand-500">
...
// == REF UNTUK MODAL SYNC 1 DOSEN ==
const isSyncModalOpen = ref(false)
const selectedDosenToSync = ref<number | ''>('')
...
const isTooltipUp = (index: number) => {
  const totalRows = paginatedGroups.value.length;
...
const handleClickOutside = (event: MouseEvent) => {
  ...
  // Event close untul dropdown Sync Modal
  if (dosenSyncDropdownContainer.value && !dosenSyncDropdownContainer.value.contains(target)) showDosenSyncDropdown.value = false
}
...
const filteredSyncDosenOptions = computed(() => {
  ...
  let available = dosenList.value.filter(dosen =>
    !dosenIdsWithKeahlian.includes(dosen.id_dosen) &&
    dosen.url_scholar &&
    dosen.url_scholar.trim() !== ''
  );

  // Pencarian dengan keyword
  if (dosenSyncSearchTerm.value) {
    const term = dosenSyncSearchTerm.value.toLowerCase();
    available = available.filter(d => d.nama_dosen.toLowerCase().includes(term));
  }
...
const openSyncModal = () => {
  selectedDosenToSync.value = '';
  dosenSyncSearchTerm.value = '';
  showDosenSyncDropdown.value = false;
...
const selectSyncDosen = (dosen: Dosen) => {
  selectedDosenToSync.value = dosen.id_dosen;
  dosenSyncSearchTerm.value = dosen.nama_dosen;
  showDosenSyncDropdown.value = false;
}

const executeSyncSingle = async () => {
  if (!selectedDosenToSync.value) return;
  await syncOneScholar(Number(selectedDosenToSync.value));
  closeSyncModal();
}
...
const syncAllScholar = async () => {
  const dosenToSync = dosenList.value.filter(d => d.url_scholar && d.grup_riset);

  if (dosenToSync.length === 0) {
    showAlert('warning', 'Peringatan', 'Tidak ada dosen yang memiliki link Scholar dan Grup Riset.');
    return;
  }
  ...
  try {
    for (let i = 0; i < dosenToSync.length; i++) {
      ...
      try {
        ...
        if (response.ok) {
          const result = await response.json();
          if (result.message && !result.message.includes('Tidak ada') && !result.message.includes('terbaru')) {
            successCount++;
          }
        }
      } catch (error) {
        console.error(`Gagal sync dosen ${dosen.nama_dosen}:`, error);
      }
...
<style scoped>
/* Biarkan style yang sudah ada sama */
