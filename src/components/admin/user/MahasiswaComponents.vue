  ...
  <div class="w-full">
    ...
    <div class="flex flex-col gap-4 mb-4">
      <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <button @click="openAddModal" class="flex items-center justify-center px-4 h-10 text-sm font-medium text-white transition rounded-lg bg-brand-500 hover:bg-brand-600 w-full sm:w-auto flex-shrink-0">
        ...
        <table class="w-full min-w-[900px] text-left border-collapse">
          <thead>
            <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-white/[0.02]">
              ...
              <th class="px-5 py-3 text-center sm:px-6 w-40"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Aksi</p></th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-if="isLoading">
              <td colspan="7" class="px-5 py-8 text-center text-gray-500 text-theme-sm">Memuat data mahasiswa...</td>
            </tr>
            ...
            <tr v-else v-for="(item, index) in paginatedList" :key="item.id" class="border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-white/[0.02]">
              ...
              <td class="px-5 py-4 sm:px-6 align-top">
                <span class="block text-gray-800 text-theme-sm dark:text-white/90 whitespace-normal break-words mt-1">{{ item.nama_mahasiswa }}</span>
              </td>
              <td class="px-5 py-4 sm:px-6 align-top">
              ...
              <td class="px-5 py-4 text-center sm:px-6 align-top">
                <span class="inline-flex items-center justify-center rounded-xl bg-brand-500/10 px-3 py-1.5 text-xs font-medium text-brand-500 whitespace-normal break-words leading-snug mt-1">
                  {{ item.topik_ta?.nama_topik || '-' }}
                </span>
              </td>
              <td class="px-5 py-4 text-center sm:px-6 align-top">
                <div class="flex items-center justify-center gap-2 mt-0.5">
                  <button @click="openEditModal(item)" class="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-gray-700 transition-colors bg-white border border-gray-200 rounded-full hover:bg-gray-50 hover:text-blue-600 shadow-theme-xs" title="Edit">
                  ...
                  <button @click="openDeleteModal(item)" class="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-gray-700 transition-colors bg-white border border-gray-200 rounded-full hover:bg-gray-50 hover:text-red-600 shadow-theme-xs" title="Hapus">
        ...
        <form @submit.prevent="submitForm" class="flex flex-col px-2">
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 mb-4">
            <div>
              ...
              <input v-model="formData.nama_mahasiswa" type="text" placeholder="Misal: Anton Surya" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-3" required />
            </div>
            <div>
              <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">NIM <span class="text-red-500">*</span></label>
              <input v-model="formData.nim" type="text" placeholder="Masukkan NIM" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-3" required />
            </div>
          </div>

          <div class="mb-4">
            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Judul Tugas Akhir</label>
            <input v-model="formData.judul_ta" type="text" placeholder="Judul TA Mahasiswa" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-3" />
          </div>
          ...
          <div>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 mb-6">
              <div>
                ...
                <input v-model="formData.username" type="text" placeholder="Username untuk mahasiswa" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-3" required />
              </div>
              <div>
                ...
                <input v-model="formData.password" type="password" placeholder="Password untuk mahasiswa" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-3" :required="!isEditing" />
          ...
          <div class="flex items-center gap-3 lg:justify-end mt-2">
            ...
            <button type="submit" :disabled="isSaving" class="flex w-full justify-center rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-brand-600 disabled:opacity-50 sm:w-auto">
    ...
    <Modal v-if="isDeleteModalOpen" @close="closeDeleteModal">
        <div class="w-full max-w-[400px] rounded-3xl bg-white p-6 dark:bg-gray-900 text-center mx-auto mt-20 z-50">
          ...
          <p class="mb-8 text-sm text-gray-500 dark:text-gray-400">
            ...
            <strong class="text-gray-800 dark:text-white text-lg">"{{ itemToDelete?.nama_mahasiswa }}"</strong>?
