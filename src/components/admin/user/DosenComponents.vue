  ...
  <div class="relative">
    ...
    <div class="flex flex-col gap-4 mb-6 p-5 rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900/50 shadow-sm relative z-20">
      ...
      <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <button v-if="isAdmin" @click="openAddModal" class="flex items-center justify-center px-5 h-10 text-sm font-medium text-white transition rounded-lg bg-brand-500 hover:bg-brand-600 shadow-theme-xs dark:bg-brand-600 dark:hover:bg-brand-500 w-full sm:w-auto">
      ...
      <div class="max-w-full overflow-x-auto custom-scrollbar">
        <table class="min-w-full">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-if="isLoading">
              <td colspan="8" class="px-5 py-8 text-center text-gray-500 text-theme-sm dark:text-gray-400">Memuat data dosen...</td>
            </tr>
            <tr v-else-if="paginatedList.length === 0">
              <td colspan="8" class="px-5 py-8 text-center text-gray-500 text-theme-sm dark:text-gray-400">Data tidak ditemukan sesuai filter.</td>
            </tr>
            ...
            <tr v-else v-for="(item, index) in paginatedList" :key="item.id_dosen" class="border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-white/[0.02]">
              ...
              <td class="px-5 py-4 sm:px-6"><span class="block text-gray-800 text-theme-sm dark:text-gray-200 font-semibold">{{ item.nama_dosen }}</span></td>
              ...
              <td class="px-5 py-4 text-center sm:px-6">
                <span class="inline-flex items-center justify-center rounded-full bg-purple-100 px-3 py-1 text-xs font-medium text-purple-800 dark:bg-purple-900/30 dark:text-purple-300 border border-purple-200 dark:border-purple-800">{{ item.grup_riset || '-' }}</span>
              </td>
              <td class="px-5 py-4 text-center sm:px-6">
                <div class="flex flex-wrap items-center justify-center gap-1">
                  <span v-if="!item.user?.role || item.user.role.length === 0" class="inline-flex items-center justify-center rounded-full bg-gray-100 px-3 py-1 text-xs font-medium text-gray-800 dark:bg-gray-800 dark:text-gray-300 border border-gray-200 dark:border-gray-700">-</span>
                  <span v-else v-for="role in item.user.role" :key="role.id_role" :class="['inline-flex items-center justify-center rounded-full px-2 py-0.5 text-xs font-medium border', role.id_role === 2 ? 'bg-blue-100 text-blue-800 border-blue-200 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-800' : 'bg-green-100 text-green-800 border-green-200 dark:bg-green-900/30 dark:text-green-300 dark:border-green-800']">
                    {{ role.nama || role.nama_role }}
                  </span>
                </div>
              </td>
              <td class="px-5 py-4 text-center sm:px-6">
                <span class="inline-flex items-center justify-center rounded-full bg-brand-500/10 px-3 py-1 text-sm font-medium text-brand-500 border border-brand-500/20 dark:bg-brand-900/30 dark:text-brand-400 dark:border-brand-800">{{ item.kuota_menguji }}</span>
              </td>

              <td v-if="isAdmin" class="px-5 py-4 text-center sm:px-6">
                <div class="flex items-center justify-center gap-2">
                  <button @click="openEditModal(item)" class="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-gray-700 transition-colors bg-white border border-gray-200 rounded-full hover:bg-gray-50 hover:text-blue-600 shadow-theme-xs dark:bg-gray-800 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-700 dark:hover:text-blue-400" title="Edit">
                  ...
                  <button @click="openDeleteModal(item)" class="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-gray-700 transition-colors bg-white border border-gray-200 rounded-full hover:bg-gray-50 hover:text-red-600 shadow-theme-xs dark:bg-gray-800 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-700 dark:hover:text-red-400" title="Hapus">
        ...
        <form @submit.prevent="submitForm" class="flex flex-col px-2">
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 mb-4">
            <div>
              ...
              <input v-model="formData.nama_dosen" type="text" placeholder="Misal: Budi Santoso, M.Kom" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-3 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200" required />
            </div>
            <div>
              <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">NIDN <span class="text-red-500">*</span></label>
              <input v-model="formData.nidn" type="text" placeholder="Masukkan NIDN" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-3 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200" required />
            </div>
          </div>
          ...
          <div>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 mb-6">
              <div>
                ...
                <input v-model="formData.username" type="text" placeholder="Username untuk dosen" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-3 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200" :required="!isEditing" />
              </div>
              <div>
                ...
                <input v-model="formData.password" type="password" placeholder="Password untuk dosen" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-3 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200" :required="!isEditing" />
          ...
          <div class="flex items-center gap-3 lg:justify-end mt-2">
            ...
            <button type="submit" :disabled="isSaving || formData.role_ids.length === 0 || formData.prodi_id === '' || formData.grup_riset === ''" class="flex w-full justify-center rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-brand-600 disabled:opacity-50 sm:w-auto dark:bg-brand-600 dark:hover:bg-brand-500">
