      ...
      <div class="max-w-full overflow-x-auto custom-scrollbar">
        <table class="min-w-full">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-if="isLoading">
              <td colspan="7" class="px-5 py-8 text-center text-gray-500 text-theme-sm">Memuat data dosen...</td>
            </tr>
            <tr v-else-if="dosenList.length === 0">
              <td colspan="7" class="px-5 py-8 text-center text-gray-500 text-theme-sm">Belum ada data dosen.</td>
            </tr>
            <tr v-else v-for="(item, index) in dosenList" :key="item.id"
              ...
              <td class="px-5 py-4 sm:px-6">
                <span class="block text-gray-800 text-theme-sm dark:text-white/90">{{ item.nama_dosen }}</span>
              </td>
              <td class="px-5 py-4 sm:px-6">
                <span class="block text-gray-500 text-theme-sm dark:text-gray-400">{{ item.nidn }}</span>
              </td>
              <td class="px-5 py-4 sm:px-6">
              ...
              <td class="px-5 py-4 text-center sm:px-6">
                <div class="flex flex-wrap items-center justify-center gap-1">
                  <span v-if="!item.user?.role || item.user.role.length === 0" class="inline-flex items-center justify-center rounded-full bg-gray-100 px-3 py-1 text-xs font-medium text-gray-800">
                    -
                  </span>
                  <span v-else v-for="role in item.user.role" :key="role.id"
                    :class="[
                      'inline-flex items-center justify-center rounded-full px-2 py-0.5 text-xs font-medium',
                      role.id === 2 ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'
                    ]">
      ...
      <div class="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11 mx-auto mt-10">
        ...
        <form @submit.prevent="submitForm" class="flex flex-col px-2">
          ...
          <div>
            ...
            <div class="mb-4">
              ...
              <div class="flex flex-col gap-3 sm:flex-row">
                <label class="relative flex cursor-pointer items-center gap-2 rounded-lg border border-gray-300 p-3 hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-white/[0.03]">
          ...
          <div class="flex items-center gap-3 lg:justify-end mt-2">
            ...
            <button type="submit" :disabled="isSaving || formData.role_ids.length === 0"
