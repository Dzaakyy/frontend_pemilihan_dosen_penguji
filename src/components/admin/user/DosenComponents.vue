<template>
  <div class="relative">
    ...
    <div
      class="flex flex-col gap-4 mb-6 p-5 rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900/50 shadow-sm relative z-20">
      ...
      <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <button v-if="isAdmin" @click="openAddModal"
          class="flex items-center justify-center px-5 h-10 text-sm font-medium text-white transition rounded-lg bg-brand-500 hover:bg-brand-600 shadow-theme-xs dark:bg-brand-600 dark:hover:bg-brand-500 w-full sm:w-auto">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div class="relative w-full" ref="grupRisetDropdownRef">
          <button type="button" @click="isGrupRisetDropdownOpen = !isGrupRisetDropdownOpen"
            ...
            <svg
              :class="['w-4 h-4 text-gray-400 transition-transform duration-200 flex-shrink-0', { 'rotate-180': isGrupRisetDropdownOpen }]"
              fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
            </svg>
          </button>
            ...
            <div v-if="isGrupRisetDropdownOpen"
              ...
              <ul class="max-h-60 overflow-y-auto custom-scrollbar py-1">
                <li @click="selectFilterGrupRiset('')"
                  class="px-4 py-2.5 text-sm cursor-pointer hover:bg-brand-50 dark:hover:bg-gray-700 text-gray-800 dark:text-gray-200 transition-colors border-b border-gray-100 dark:border-gray-700 font-medium">
                  Semua Grup Riset</li>
                <li v-for="grup in dataGrupRiset" :key="grup" @click="selectFilterGrupRiset(grup)"
                  class="px-4 py-2.5 text-sm cursor-pointer hover:bg-brand-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 transition-colors border-b border-gray-50 last:border-0 dark:border-gray-700/50">
                  {{ grup }}</li>
              </ul>
            </div>
    ...
    <!-- Tabel Data -->
    <div
      class="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900/50 relative z-10">
      <div class="max-w-full overflow-x-auto custom-scrollbar">
        <table class="min-w-full">
          <thead>
            <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-white/[0.02]">
              ...
              <th class="px-5 py-3 text-center sm:px-6">
                <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Grup Riset (Center)</p>
              </th>
              <th class="px-5 py-3 text-center sm:px-6">
                <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Peran/Role</p>
              </th>
              <th class="px-5 py-3 text-center sm:px-6">
                <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Kuota Menguji</p>
              </th>
              <th v-if="isAdmin" class="px-5 py-3 text-center w-32 sm:px-6">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-if="isLoading">
              <td colspan="8" class="px-5 py-8 text-center text-gray-500 text-theme-sm dark:text-gray-400">Memuat data
                dosen...</td>
            </tr>
            <tr v-else-if="paginatedList.length === 0">
              <td colspan="8" class="px-5 py-8 text-center text-gray-500 text-theme-sm dark:text-gray-400">Data tidak
                ditemukan sesuai filter.</td>
            </tr>

            <tr v-else v-for="(item, index) in paginatedList" :key="item.id_dosen"
              class="border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-white/[0.02]">
              <td class="px-5 py-4 sm:px-6">
                <span class="block font-medium text-gray-800 text-theme-sm dark:text-gray-300">
                  {{ (currentPage - 1) * itemsPerPage + index + 1 }}
                </span>
              </td>
              <td class="px-5 py-4 sm:px-6"><span
                  class="block text-gray-800 text-theme-sm dark:text-gray-200 font-semibold">{{ item.nama_dosen
                  }}</span></td>
              <td class="px-5 py-4 sm:px-6"><span class="block text-gray-500 text-theme-sm dark:text-gray-400">{{
                  item.nidn }}</span></td>
              <td class="px-5 py-4 sm:px-6"><span class="block text-gray-500 text-theme-sm dark:text-gray-400">{{
                item.prodi?.nama_prodi || '-' }}</span></td>
              <td class="px-5 py-4 text-center sm:px-6">
                <span
                  class="inline-flex items-center justify-center rounded-full bg-purple-100 px-3 py-1 text-xs font-medium text-purple-800 dark:bg-purple-900/30 dark:text-purple-300 border border-purple-200 dark:border-purple-800">{{
                    item.grup_riset || '-' }}</span>
              </td>
              <td class="px-5 py-4 text-center sm:px-6">
                <div class="flex flex-wrap items-center justify-center gap-1">
                  <span v-if="!item.user?.role || item.user.role.length === 0"
                    class="inline-flex items-center justify-center rounded-full bg-gray-100 px-3 py-1 text-xs font-medium text-gray-800 dark:bg-gray-800 dark:text-gray-300 border border-gray-200 dark:border-gray-700">-</span>
                  <span v-else v-for="role in item.user.role" :key="role.id_role"
                    :class="['inline-flex items-center justify-center rounded-full px-2 py-0.5 text-xs font-medium border', role.id_role === 2 ? 'bg-blue-100 text-blue-800 border-blue-200 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-800' : 'bg-green-100 text-green-800 border-green-200 dark:bg-green-900/30 dark:text-green-300 dark:border-green-800']">
              ...
              <td class="px-5 py-4 text-center sm:px-6">
                <span
                  class="inline-flex items-center justify-center rounded-full bg-brand-500/10 px-3 py-1 text-sm font-medium text-brand-500 border border-brand-500/20 dark:bg-brand-900/30 dark:text-brand-400 dark:border-brand-800">{{
                    item.kuota_menguji }}</span>
              </td>
    ...
    <Modal v-if="isModalOpen" @close="closeModal">
      <div
        class="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 border dark:border-gray-700 lg:p-11 mx-auto mt-10 shadow-2xl">
        <button @click="closeModal"
          class="transition-color absolute right-5 top-5 z-[100] flex h-11 w-11 items-center justify-center rounded-full bg-gray-100 text-gray-400 hover:bg-gray-200 hover:text-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-200">
        ...
        <form @submit.prevent="submitForm" class="flex flex-col px-2">
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 mb-4">
            <div>
              ...
              <input v-model="formData.nama_dosen" type="text" placeholder="Misal: Budi Santoso, M.Kom"
                class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-3 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200"
                required />
            </div>
            <div>
              <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">NIDN <span
                  class="text-red-500">*</span></label>
              <input v-model="formData.nidn" type="text" placeholder="Masukkan NIDN"
                class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-3 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200"
                required />
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 mb-4">
            ...
            <div class="relative w-full" ref="formGrupRisetDropdownRef">
              <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Grup Riset (Center) <span
                  class="text-red-500">*</span></label>
              <button type="button" @click="isFormGrupRisetDropdownOpen = !isFormGrupRisetDropdownOpen"
                class="flex items-center justify-between h-11 w-full rounded-lg border border-gray-300 bg-white px-4 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:outline-none focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200 text-left transition-colors">
                <span class="truncate pr-4" :class="{ 'text-gray-500 dark:text-gray-400': !formData.grup_riset }">{{
                  selectedFormGrupRisetLabel }}</span>
                <svg
                  :class="['w-4 h-4 text-gray-400 transition-transform duration-200 flex-shrink-0', { 'rotate-180': isFormGrupRisetDropdownOpen }]"
                  fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                </svg>
              </button>
              <transition enter-active-class="transition duration-100 ease-out"
                ...
                leave-active-class="transition duration-75 ease-in" leave-from-class="transform scale-100 opacity-100"
                leave-to-class="transform scale-95 opacity-0">
                <div v-if="isFormGrupRisetDropdownOpen"
                  class="absolute z-[100] w-full mt-1.5 top-full bg-white border border-gray-200 rounded-lg shadow-xl dark:bg-gray-800 dark:border-gray-700 overflow-hidden">
                  <ul class="max-h-48 overflow-y-auto custom-scrollbar py-1">
                    <li v-for="grup in dataGrupRiset" :key="grup" @click="selectFormGrupRiset(grup)"
                      class="px-4 py-2.5 text-sm cursor-pointer hover:bg-brand-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 transition-colors border-b border-gray-50 last:border-0 dark:border-gray-700/50">
                      {{ grup }}
                    </li>
                  </ul>
          ...
          <div class="mb-6">
            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Kuota Menguji <span
                class="text-red-500">*</span></label>
            <input v-model="formData.kuota_menguji" type="number" placeholder="Misal: 5"
              class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-3 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200"
              required />
          </div>

          <div>
            <h5 class="mb-3 text-lg font-medium text-gray-800 dark:text-white/90 border-t pt-4 dark:border-gray-700">
              Akun Pengguna (Login)</h5>

            <div class="mb-4">
              <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Peran (Role) Akun <span
                  class="text-red-500">*</span></label>
              <div class="flex flex-col gap-3 sm:flex-row">
                <label
                  class="relative flex cursor-pointer items-center gap-2 rounded-lg border border-gray-300 p-3 hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800">
                  <input type="checkbox" v-model="formData.role_ids" :value="2"
                    class="h-5 w-5 rounded border-gray-300 text-brand-500 focus:ring-brand-500 dark:bg-gray-900 dark:border-gray-600" />
                  <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Ketua Prodi</span>
                </label>
                <label
                  class="relative flex cursor-pointer items-center gap-2 rounded-lg border border-gray-300 p-3 hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800">
                  <input type="checkbox" v-model="formData.role_ids" :value="3"
                    class="h-5 w-5 rounded border-gray-300 text-brand-500 focus:ring-brand-500 dark:bg-gray-900 dark:border-gray-600" />
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 mb-6">
              <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Username <span
                    class="text-red-500">*</span></label>
                <input v-model="formData.username" type="text" placeholder="Username untuk dosen"
                  class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-3 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200"
                  :required="!isEditing" />
              </div>
              <div>
                ...
                <input v-model="formData.password" type="password" placeholder="Password untuk dosen"
                  class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-3 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200"
                  :required="!isEditing" />
          ...
          <div class="flex items-center gap-3 lg:justify-end mt-2">
            ...
            <button type="submit"
              :disabled="isSaving || formData.role_ids.length === 0 || formData.prodi_id === '' || formData.grup_riset === ''"
    ...
    <Modal v-if="isDeleteModalOpen" @close="closeDeleteModal">
      <div
        class="w-full max-w-[400px] rounded-3xl bg-white p-6 text-center mx-auto mt-20 z-50 shadow-xl dark:bg-gray-900 border dark:border-gray-700">
