<template>
  <div class="relative">
    <div v-if="alert.show"
      class="fixed top-20 right-5 z-[9999999] w-full max-w-sm transition-all duration-300 ease-in-out">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <div class="relative w-full" ref="kaprodiDropdownRef">
          <button type="button" @click="isKaprodiDropdownOpen = !isKaprodiDropdownOpen"
            ...
            <svg
              :class="['w-4 h-4 text-gray-400 transition-transform duration-200 flex-shrink-0', { 'rotate-180': isKaprodiDropdownOpen }]"
              fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
            </svg>
          </button>
          <transition enter-active-class="transition duration-100 ease-out"
            ...
            leave-active-class="transition duration-75 ease-in" leave-from-class="transform scale-100 opacity-100"
            leave-to-class="transform scale-95 opacity-0">
            <div v-if="isKaprodiDropdownOpen"
              class="absolute z-[100] w-full mt-1.5 top-full bg-white border border-gray-200 rounded-lg shadow-xl dark:bg-gray-800 dark:border-gray-700 overflow-hidden">
              <ul class="max-h-60 overflow-y-auto custom-scrollbar py-1">
                <li @click="selectFilterKaprodi('')"
                  class="px-4 py-2.5 text-sm cursor-pointer hover:bg-brand-50 dark:hover:bg-gray-700 text-gray-800 dark:text-gray-200 transition-colors border-b border-gray-100 dark:border-gray-700 font-medium">
                  Dosen</li>
                <li @click="selectFilterKaprodi(true)"
                  class="px-4 py-2.5 text-sm cursor-pointer hover:bg-brand-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 transition-colors border-b border-gray-50 last:border-0 dark:border-gray-700/50">
                  Kaprodi</li>
              </ul>
            </div>
      ...
      <div class="max-w-full overflow-x-auto custom-scrollbar">
        <table class="min-w-full">
          <thead>
            <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-white/[0.02]">
              ...
              <th class="px-5 py-3 text-center sm:px-6">
                <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Grup Riset</p>
              </th>
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-if="isLoading">
              <td colspan="7" class="px-5 py-8 text-center text-gray-500 text-theme-sm dark:text-gray-400">Memuat data
                dosen...</td>
            </tr>
            <tr v-else-if="paginatedList.length === 0">
              <td colspan="7" class="px-5 py-8 text-center text-gray-500 text-theme-sm dark:text-gray-400">Data tidak
            ...
            <tr v-else v-for="(item, index) in paginatedList" :key="item.id_dosen"
              ...
              <td class="px-5 py-4 sm:px-6 align-top">
                <span class="block text-gray-800 text-theme-sm dark:text-gray-200 font-semibold">{{ item.nama_dosen
                }}</span>
                <a v-if="item.url_scholar" :href="item.url_scholar" target="_blank"
                  class="inline-flex items-center text-[11px] text-blue-500 hover:text-blue-600 mt-1 dark:text-blue-400 dark:hover:text-blue-300">
              ...
              <td class="px-5 py-4 sm:px-6 align-top"><span
                  class="block text-gray-500 text-theme-sm dark:text-gray-400 mt-1">{{ item.nidn }}</span></td>
              <td class="px-5 py-4 sm:px-6 align-top"><span
                  class="block text-gray-500 text-theme-sm dark:text-gray-400 mt-1">{{ item.prodi?.nama_prodi || '-'
                  }}</span></td>
              <td class="px-5 py-4 text-center sm:px-6 align-top">
                <span
                  class="inline-flex items-center justify-center rounded-full bg-purple-100 px-3 py-1 text-xs font-medium text-purple-800 dark:bg-purple-900/30 dark:text-purple-300 border border-purple-200 dark:border-purple-800 mt-1">{{
        ...
        <div class="flex flex-col gap-6" v-if="selectedDosenDetail">
          ...
          <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 border-t border-gray-100 pt-6 dark:border-gray-800">
            <div class="flex flex-col">
              ...
              <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{
                selectedDosenDetail.prodi?.nama_prodi
                || '-' }}</span>
            </div>
            <div class="flex flex-col">
          ...
          <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 border-t border-gray-100 pt-6 dark:border-gray-800">
            <div class="flex flex-col">
              ...
              <span class="text-sm font-bold text-gray-800 dark:text-gray-200">{{ selectedDosenDetail.user?.username ||
                '-' }}</span>
            </div>
            <div class="flex flex-col">
              <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Alamat Email</span>
              <span class="text-sm font-bold text-gray-800 dark:text-gray-200">{{ selectedDosenDetail.user?.email || '-'
              }}</span>
            </div>
          </div>
          ...
          <div class="flex flex-col border-t border-gray-100 pt-6 dark:border-gray-800">
            ...
            <div v-if="selectedDosenDetail.url_scholar"
              ...
              <div class="flex-1 min-w-0">
                ...
                <span class="text-xs text-gray-500 dark:text-gray-400">Klik tautan untuk mengunjungi profil Google
                  Scholar
                  dosen ini.</span>
              </div>
            </div>
    ...
    <Modal v-if="isModalOpen" @close="closeModal">
      <div
            ...
            <div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-5 gap-y-4">
                ...
                <div class="relative w-full" ref="formGrupRisetDropdownRef">
                  <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Grup Riset (Center)
                ...
                <div class="sm:col-span-2">
                  <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">URL Google
                    Scholar</label>
                  <input v-model="formData.url_scholar" type="url" placeholder="https://scholar.google.co.id/..."
                    class="h-10 w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200 transition-colors" />
                </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-3 gap-x-5 gap-y-4">
                <div>
                  <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Email <span
                      class="text-red-500">*</span></label>
                  <input v-model="formData.email" type="email" placeholder="email@gmail.com"
                    class="h-10 w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200 transition-colors"
                    required />
                </div>
        ...
        <div
          ...
          <button type="submit" form="dosenForm"
