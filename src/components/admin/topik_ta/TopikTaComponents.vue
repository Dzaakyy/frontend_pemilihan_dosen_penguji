      ...
      <div class="max-w-full overflow-x-auto custom-scrollbar">
        <table class="min-w-full">
          <thead>
            <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-white/[0.02]">
              ...
              <th class="px-5 py-3 text-left sm:px-6">
                <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Nama Topik TA</p>
              </th>
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tr v-else-if="topikList.length === 0">
              <td colspan="3" class="px-5 py-8 text-center text-gray-500 text-theme-sm">
        ...
        <div
          ...
          <div class="px-2 pr-14 mb-6 mt-4">
            <h4 class="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
              {{ isEditing ? 'Edit Topik TA' : 'Tambah Topik TA' }}
            </h4>
          </div>
          ...
          <form @submit.prevent="submitForm" class="flex flex-col px-2">
            <div class="mb-6">
              <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                Nama Topik <span class="text-red-500">*</span>
              </label>
              <input v-model="formData.nama_topik" type="text" placeholder="Misal: Sistem Pakar"
    ...
    <Modal v-if="isDeleteModalOpen" @close="closeDeleteModal">
        <div class="w-full max-w-[400px] rounded-3xl bg-white p-6 dark:bg-gray-900 text-center mx-auto mt-20 z-50">
          ...
          <p class="mb-8 text-sm text-gray-500 dark:text-gray-400">
            Apakah Anda yakin ingin menghapus data topik TA
            <br /> <strong class="text-gray-800 dark:text-white text-lg">"{{ itemToDelete?.nama_topik }}"</strong>?
...
const itemToDelete = ref<Topik | null>(null)
...
const submitForm = async () => {
  ...
  try {
    ...
    const response = await fetch(url, {
      ...
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({ nama_topik: formData.value.nama_topik })
    })

    if (response.ok) {
      closeModal()
      fetchTopik()
      showAlert('success', 'Berhasil!', `Data Topik TA berhasil ${isEditing.value ? 'diperbarui' : 'ditambahkan'}.`)
    } else {
...
// --- FUNGSI DELETE MODAL ---
const openDeleteModal = (item: Topik) => {
...
const confirmDelete = async () => {
  ...
  try {
    ...
    if (response.ok) {
      closeDeleteModal()
      fetchTopik()
      showAlert('success', 'Dihapus!', 'Data Topik TA berhasil dihapus permanen.')
    } else {
