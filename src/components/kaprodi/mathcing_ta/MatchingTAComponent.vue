<template>
  <div class="relative">
    ...
    <div class="flex flex-col gap-4 mb-6 p-5 rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900/50 shadow-sm relative">
      <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3 w-full sm:w-auto">
          ...
          <button v-if="filteredGroups.length > 0" @click="exportToExcel" class="flex-1 sm:flex-none flex items-center justify-center px-4 h-10 text-sm font-medium text-emerald-700 transition rounded-lg bg-emerald-50 border border-emerald-200 hover:bg-emerald-100 shadow-theme-xs dark:bg-emerald-900/20 dark:border-emerald-800/50 dark:text-emerald-400 dark:hover:bg-emerald-900/40">
          ...
          <span v-else-if="canManageMatching && !hasUnassignedMahasiswa" class="text-sm font-medium text-emerald-600 flex items-center bg-emerald-50 px-4 py-2 rounded-lg border border-emerald-100 dark:bg-emerald-900/20 dark:border-emerald-800/50 dark:text-emerald-400">
      ...
      <div v-if="!isAdmin && activeProdiName" class="w-full mb-2">
        <span class="inline-flex items-center px-3 py-1 rounded-md bg-blue-50 border border-blue-100 text-blue-700 text-xs font-bold dark:bg-blue-900/20 dark:border-blue-800/50 dark:text-blue-400">
    ...
    <div class="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900/50 shadow-sm">
        ...
        <table class="min-w-full border-collapse">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <template v-else v-for="group in paginatedGroups" :key="group.mahasiswa_id">
              <tr v-for="(item, index) in group.rekomendasi" :key="item.id_rekomendasi" class="hover:bg-gray-50 dark:hover:bg-white/[0.02]">

                <td v-if="index === 0" :rowspan="group.rekomendasi.length" class="px-5 py-5 border-r border-gray-200 dark:border-gray-700 align-top bg-white dark:bg-gray-800/30">
                ...
                <td class="px-5 py-3 text-center align-middle">
                  <span class="inline-flex items-center justify-center rounded-md bg-blue-50 px-2.5 py-1 text-xs font-semibold text-blue-600 border border-blue-100 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800">
                    {{ item.iterasi_pso }}
                  </span>
                </td>

                <td class="px-5 py-3 text-center align-middle">
                  <span class="font-mono text-gray-800 dark:text-gray-200 font-bold text-sm bg-gray-100 dark:bg-gray-800 px-3 py-1.5 rounded-md shadow-sm border border-gray-200 dark:border-gray-700">
                    {{ item.nilai_fitness }}%
                  </span>
                </td>
              </tr>
    ...
    <Modal v-if="isGenerateModalOpen" @close="closeGenerateModal">
      <div class="relative w-full max-w-[500px] rounded-3xl bg-white p-6 dark:bg-gray-900 mx-auto mt-20 shadow-2xl border dark:border-gray-800">
        <div class="text-center mb-6">
          <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-brand-100 text-brand-600 mb-4 shadow-inner dark:bg-brand-900/30 dark:text-brand-400">
        ...
        <form @submit.prevent="submitGeneratePso" class="flex flex-col gap-4 text-left">
          <div class="grid grid-cols-2 gap-4">
             <div>
               ...
               <input v-model.number="psoParams.maxIter" type="number" class="w-full h-10 px-3 rounded-lg border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-800 dark:text-gray-200 text-sm focus:ring-2 focus:ring-brand-500 outline-none" required />
             </div>
             <div>
               <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1">Jumlah Partikel</label>
               <input v-model.number="psoParams.numParticles" type="number" class="w-full h-10 px-3 rounded-lg border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-800 dark:text-gray-200 text-sm focus:ring-2 focus:ring-brand-500 outline-none" required />
             </div>
          </div>
          <div class="grid grid-cols-3 gap-3">
             <div>
               <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1">Bobot Inertia (w)</label>
               <input v-model.number="psoParams.w" type="number" step="0.1" class="w-full h-10 px-3 rounded-lg border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-800 dark:text-gray-200 text-sm focus:ring-2 focus:ring-brand-500 outline-none" required />
             </div>
             <div>
               <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1">Bobot Kognitif (c1)</label>
               <input v-model.number="psoParams.c1" type="number" step="0.1" class="w-full h-10 px-3 rounded-lg border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-800 dark:text-gray-200 text-sm focus:ring-2 focus:ring-brand-500 outline-none" required />
             </div>
             <div>
               <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1">Bobot Sosial (c2)</label>
               <input v-model.number="psoParams.c2" type="number" step="0.1" class="w-full h-10 px-3 rounded-lg border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-800 dark:text-gray-200 text-sm focus:ring-2 focus:ring-brand-500 outline-none" required />
             </div>
          </div>

          <div class="flex items-center gap-3 w-full mt-4 border-t border-gray-100 pt-5 dark:border-gray-800">
            <button @click="closeGenerateModal" type="button" class="flex-1 justify-center rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-700 transition-colors">
              Batal
            </button>
            <button type="submit" :disabled="isGenerating" class="flex-1 flex justify-center items-center rounded-lg bg-brand-500 px-4 py-3 text-sm font-bold text-white hover:bg-brand-600 disabled:opacity-50 transition-all shadow-md hover:shadow-lg dark:bg-brand-600 dark:hover:bg-brand-500">
      ...
      <div class="relative w-full max-w-[800px] flex flex-col max-h-[90vh] rounded-3xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 mx-auto mt-10 shadow-2xl overflow-hidden transition-colors">
        ...
        <div class="px-6 py-6 overflow-y-auto custom-scrollbar flex-1">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div class="p-4 bg-brand-50 border border-brand-100 rounded-xl dark:bg-brand-900/20 dark:border-brand-800/50">
              <span class="block text-[11px] font-bold text-brand-600 dark:text-brand-400 uppercase tracking-wider mb-1">Mahasiswa</span>
              <p class="font-bold text-gray-800 dark:text-white/90">{{ selectedGroupDetail?.nama_mahasiswa }}</p>
              <p class="text-xs text-gray-500 dark:text-gray-400 line-clamp-2 mt-1" :title="selectedGroupDetail?.judul_ta">{{ selectedGroupDetail?.judul_ta }}</p>
            </div>

            <div class="p-4 bg-emerald-50 border border-emerald-100 rounded-xl dark:bg-emerald-900/20 dark:border-emerald-800/50">
              <span class="block text-[11px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider mb-1">Rekomendasi Dosen (Rank #{{ selectedRekomendasiDetail?.rank }})</span>
              <p class="font-bold text-gray-800 dark:text-white/90">{{ selectedRekomendasiDetail?.dosen?.nama_dosen }}</p>
              <div class="flex items-center gap-2 mt-2">
                <span class="text-xs font-bold text-emerald-700 bg-emerald-100 dark:bg-emerald-900/40 dark:text-emerald-400 px-2 py-1 rounded">Skor Total: {{ selectedRekomendasiDetail?.nilai_fitness }}%</span>
                <span class="text-xs font-bold text-blue-700 bg-blue-100 dark:bg-blue-900/40 dark:text-blue-400 px-2 py-1 rounded">Ditemukan di Iterasi: {{ selectedRekomendasiDetail?.iterasi_pso }}</span>
              </div>
            </div>
          </div>
          ...
          <div v-if="parsedDetailKriteria" class="border border-gray-200 dark:border-gray-700 rounded-xl p-4 bg-gray-50 dark:bg-gray-800/50 mb-6">
            <h5 class="font-bold text-gray-800 dark:text-white/90 mb-3 text-sm flex items-center gap-2">
              <svg class="w-4 h-4 text-brand-500 dark:text-brand-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
            ...
            <div class="space-y-4">
              <div class="flex flex-col bg-white dark:bg-gray-800 p-3 rounded border border-gray-100 dark:border-gray-700">
                <div class="flex justify-between items-center mb-2">
                  ...
                  <span class="font-bold text-blue-600 dark:text-blue-400">+ {{ parsedDetailKriteria.poin_teks }} Poin</span>
                </div>

                <div class="bg-gray-50 dark:bg-gray-900/50 p-2 rounded text-xs border border-gray-100 dark:border-gray-700">
                  ...
                  <div class="flex flex-wrap gap-1">
                    ...
                    <span v-else v-for="kata in parsedDetailKriteria.matched_words" :key="kata" class="font-bold bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-400 px-1.5 py-0.5 rounded">
                      {{ kata }}
                    </span>
                  </div>
                </div>
              ...
              <div class="flex flex-col sm:flex-row justify-between sm:items-center bg-white dark:bg-gray-800 p-3 rounded border border-gray-100 dark:border-gray-700">
                ...
                <span class="font-bold text-orange-500 dark:text-orange-400 mt-1 sm:mt-0">+ {{ parsedDetailKriteria.poin_kuota }} Poin</span>
              </div>

              <div class="flex flex-col sm:flex-row justify-between sm:items-center bg-indigo-50 dark:bg-indigo-900/20 p-3 rounded border border-indigo-100 dark:border-indigo-800/50">
          ...
          <div class="border border-gray-200 dark:border-gray-700 rounded-xl p-4 bg-white dark:bg-gray-800">
            <div class="mb-4">
              ...
              <p class="text-xs text-gray-500 dark:text-gray-400">Rekam jejak asli partikel menemukan nilai fitness tertinggi (Global Best) selama iterasi pencarian solusi.</p>
            </div>
...
const openDetailModal = (item: Rekomendasi, group: GroupedMahasiswa) => {
  ...
  // KEMBALIKAN TEMA DINAMIS AGAR DETEKSI GELAP/TERANG KEMBALI BEKERJA
  chartOptions.value = {
    ...chartOptions.value,
...
/* ========================================================
   PERBAIKAN CSS TOOLTIP APEXCHARTS - TEMA GELAP & TERANG
   Menggunakan :global() agar CSS menembus DOM ApexCharts
   ======================================================== */

/* 1. KOTAK UTAMA (WRAPPER) */
...
/* 2. WARNA LATAR & BORDER (LIGHT MODE) */
:global(.apexcharts-tooltip),
...
/* 3. WARNA LATAR & BORDER (DARK MODE) */
...
/* 4. BAGIAN JUDUL (ITERASI) - HAPUS ABU-ABU */
